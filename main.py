...
def compute_flow_seg(video,H,start):
    ...
    grid=Variable(torch.from_numpy(grid).float().cuda(),requires_grad=True)
...
def compute_H(path):
    ...
    return np.concatenate((np.expand_dims(np.eye(3),0),H),0)
...
with torch.no_grad():
    ...
    grid_large=Variable(torch.from_numpy(grid_large).float().cuda(),requires_grad=True)
    ...
    for ite2 in range(int(nseg)):                
        ...
        MASK=MASK[:result.shape[0]-nframe-1,:,:,:]
